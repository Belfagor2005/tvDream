...
TMPPATH=/tmp/tvDream-install
FILEPATH=/tmp/tvDream-main.tar.gz

echo "Starting tvDream installation..."
...
cleanup() {
    ...
    [ -d "/tmp/tvDream-main" ] && rm -rf "/tmp/tvDream-main"
}
...
echo "⬇️ Downloading tvDream..."
wget --no-check-certificate 'https://github.com/Belfagor2005/tvDream/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "❌ Failed to download tvDream package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract tvDream package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/tvDream-main/usr/lib/enigma2/python/Plugins/Extensions/tvDream" ]; then
    cp -r "$TMPPATH/tvDream-main/usr/lib/enigma2/python/Plugins/Extensions/tvDream"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/tvDream-main/usr/lib64/enigma2/python/Plugins/Extensions/tvDream" ]; then
    cp -r "$TMPPATH/tvDream-main/usr/lib64/enigma2/python/Plugins/Extensions/tvDream"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/tvDream-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/tvDream-main/usr"/* /usr/ 2>/dev/null
