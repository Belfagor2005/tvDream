...
exec > >(tee -a /tmp/tvDream_debug.txt) 2>&1
set -x
...
wget --no-check-certificate 'https://github.com/Belfagor2005/tvDream/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
...
IMAGE VERSION: $distro_version" >> /tmp/tvDream_debug.txt
sleep 5
killall -9 enigma2
exit 0